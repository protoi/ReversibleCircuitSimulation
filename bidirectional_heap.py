...
def greedily_pick_best_fit(fault_data: dict[int, set[int]]) -> list[int]:
    ...
    class heapElement:
        def __init__(self, circ_input, faults):
    ...
    while bool(heap_input_fault_mapping):
        ...
        most_faults_identified = heappop(heap_input_fault_mapping)
        answer.append(most_faults_identified.circuit_input)

        selection.update(most_faults_identified.faults)

        for index, element in enumerate(heap_input_fault_mapping):
            heap_input_fault_mapping[index].faults = element.faults.difference(most_faults_identified.faults)
            if not bool(heap_input_fault_mapping[index].faults):
                heap_input_fault_mapping[index].faults = None

        heap_input_fault_mapping = [x for x in heap_input_fault_mapping if x.faults is not None]
        heapify(heap_input_fault_mapping)
